...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  const [authState, setAuthState] = useState<TAuthState>({ token: '', authenticated: false });
  ...
  const register = async (
    ...
    if (!firstName || !lastName || !email || !password) return wariningToast('All Fields are required');
    try {
      const payload = {
        email,
        firstName,
  ...
  const login = async (email: string, password: string) => {
    if (!email || !password) return wariningToast('Please enter Login information');
    try {
